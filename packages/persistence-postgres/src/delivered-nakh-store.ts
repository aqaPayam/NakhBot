...
function commandHash(
  command: ViewNakhProfileCommand | AcceptNakhCommand | RejectNakhCommand,
): string {
...
export class PostgresDeliveredNakhStore implements DeliveredNakhReadStore, NakhReceiverActionStore {
  ...
  public async accept(write: AcceptNakhWrite): Promise<NakhActionResult> {
    const { command } = write;
    const receiverUserId = command.actor.userId;
    const requestHash = commandHash(command);
    const locator = await this.database
    ...
    return this.database.transaction().execute(async (transaction) => {
      ...
      if (
        ...
        participants.some(
          (participant) =>
            participant.state !== 'active' || participant.completion_status !== 'complete',
        )
      )
        throw new ApplicationError('nakh_unavailable', 'error.nakh.unavailable', 409);
      ...
      const rejection = await transaction
        ...
        .executeTakeFirst();
      if (pairState !== undefined || rejection !== undefined)
        throw new ApplicationError('nakh_unavailable', 'error.nakh.unavailable', 409);
      ...
      await transaction
        ...
        .values({
          ...
          source: 'nakh_accept',
          source_like_a_id: null,
          source_like_b_id: null,
          source_nakh_id: nakh.id,
          status: 'active',
          created_at: now,
          closed_at: null,
        })
      ...
      const activeLikeIds = likes.filter((like) => like.status === 'active').map((like) => like.id);
      if (activeLikeIds.length !== 0) {
        const closedLikes = await transaction
          ...
          .returning('id')
          .execute();
        if (closedLikes.length !== activeLikeIds.length)
          throw new ApplicationError('internal_error', 'error.internal', 500);
        await transaction
          .insertInto('platform.outbox_events')
          .values({
            ...
            schema_version: 1,
            payload: { likeIds: activeLikeIds, reason: 'matched', matchId: write.matchId },
            occurred_at: now,
            available_at: now,
            published_at: null,
            last_error_code: null,
      ...
      await transaction
        .updateTable('nakh.nakhes')
        .set({ status: 'accepted', accepted_at: now, version: nakh.version + 1 })
        .where('id', '=', nakh.id)
      ...
      await transaction
        ...
        .values([
          {
            id: write.nakhEventId,
            aggregate_type: 'nakh',
            aggregate_id: nakh.id,
            event_type: 'nakh.status-changed.v1',
            schema_version: 1,
            payload: { nakhId: nakh.id, status: 'accepted', matchId: write.matchId },
            occurred_at: now,
            available_at: now,
            published_at: null,
            last_error_code: null,
      ...
      for (const userId of [pair.userLowId, pair.userHighId]) {
        await insertNotification(transaction, {
          userId,
          type: 'match_created',
          titleKey: 'matching.match.created',
          bodyKey: 'nakh.action.accepted',
          payload: { matchId: write.matchId },
          deduplicationKey: `match-created:${write.matchId}:${userId}`,
      ...
      await transaction
        ...
        .values({
          ...
          actor_admin_id: null,
          subject_type: 'match',
          subject_id: write.matchId,
          result_code: 'matched',
          metadata_schema_version: 1,
          metadata: { nakhId: nakh.id },
          request_id: command.requestId,
